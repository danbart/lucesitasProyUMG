...
require_once("include/EstuHistSocial_variables.php");
...
$page_layouts["EstuHistSocial_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=EstuHistSocial.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=EstuHistSocial.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
if($mastertable == "lcs_centro")
{
	...
	$dparams = array("detailtable" => "EstuHistSocial", "keys" => $masterkeys);
	$master = array(); 
