...
$page_layouts["lcs_estudiante_export"] = $layout;
...
if( @$_REQUEST["a"] != "" )
{
	...
	if (@$_REQUEST["mdelete"])
	{
		...
		{
			$keys=array();
			$keys["Nombre"] = refine($_REQUEST["mdelete1"][mdeleteIndex($ind)]);
			$keys["Apellido"] = refine($_REQUEST["mdelete2"][mdeleteIndex($ind)]);
			$keys["Direcccion"] = refine($_REQUEST["mdelete3"][mdeleteIndex($ind)]);
			$keys["FNacimiento"] = refine($_REQUEST["mdelete4"][mdeleteIndex($ind)]);
			$keys["FotoRostro"] = refine($_REQUEST["mdelete5"][mdeleteIndex($ind)]);
			$keys["Ingresado"] = refine($_REQUEST["mdelete6"][mdeleteIndex($ind)]);
			$keys["_idCentro"] = refine($_REQUEST["mdelete7"][mdeleteIndex($ind)]);
			$selected_recs[] = $keys;
		}
	}
	elseif(@$_REQUEST["selection"])
	{
		foreach(@$_REQUEST["selection"] as $keyblock)
		{
			...
			$keys = array();
			$keys["Nombre"] = urldecode($arr[0]);
			$keys["Apellido"] = urldecode($arr[1]);
			$keys["Direcccion"] = urldecode($arr[2]);
			$keys["FNacimiento"] = urldecode($arr[3]);
			$keys["FotoRostro"] = urldecode($arr[4]);
			$keys["Ingresado"] = urldecode($arr[5]);
			$keys["_idCentro"] = urldecode($arr[6]);
			$selected_recs[] = $keys;
		}
	}
