...
$page_layouts["lcs_historialclinico_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["idHistorialClinico"]));
	
	
	//	idHistorialClinico - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idHistorialClinico", $data, $keylink);
			$row["idHistorialClinico_value"] = $value;
			$format = $pSet->getViewFormat("idHistorialClinico");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idHistorialClinico")))
				$class = ' rnr-field-number';
			$row["idHistorialClinico_class"] = $class;
...
$xt->load_template(GetTemplateName("lcs_historialclinico", "detailspreview"));
