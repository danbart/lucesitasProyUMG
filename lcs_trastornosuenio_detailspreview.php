...
require_once("include/lcs_trastornosuenio_variables.php");
...
$page_layouts["lcs_trastornosuenio_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["idTrastornoSuenio"]));
	
	
	//	idTrastornoSuenio - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idTrastornoSuenio", $data, $keylink);
			$row["idTrastornoSuenio_value"] = $value;
			$format = $pSet->getViewFormat("idTrastornoSuenio");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idTrastornoSuenio")))
				$class = ' rnr-field-number';
			$row["idTrastornoSuenio_class"] = $class;
	//	TipoTSueño - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TipoTSueño", $data, $keylink);
			$row["TipoTSue_o_value"] = $value;
			$format = $pSet->getViewFormat("TipoTSueño");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TipoTSueño")))
				$class = ' rnr-field-number';
			$row["TipoTSue_o_class"] = $class;
...
$xt->load_template(GetTemplateName("lcs_trastornosuenio", "detailspreview"));
