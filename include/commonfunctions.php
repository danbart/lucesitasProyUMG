...
function checkTableName($shortTName, $type=false)
{
	...
	if ("lcs_terapias" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_terapista" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_tiposocial" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_transtornoalimenticio" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_transtornoesfinteres" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_trastornosuenio" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_lenguaje" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_historialclinico" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_histoiralfamiliar" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_enfermedades" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_embarazo" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_espemed" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_descsocial" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_encargado" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_emociones" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	if ("lcs_medicamento" == $shortTName && ($type===false || ($type!==false && $type == 0)))
		return true;
	...
	if ("RepEstudMed" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("EstuHistSocial" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("TranstornosAlimenticio" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("EstuHistLengua" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("TranstornosEsfinteres" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("TranstornosSueno" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
	if ("EstuHistEmoci" == $shortTName && ($type===false || ($type!==false && $type == 2)))
		return true;
