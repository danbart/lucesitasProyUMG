...
require_once("include/EstuHistEmoci_variables.php");
...
$page_layouts["EstuHistEmoci_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=EstuHistEmoci.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=EstuHistEmoci.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
if($mastertable == "lcs_centro")
{
	...
	$dparams = array("detailtable" => "EstuHistEmoci", "keys" => $masterkeys);
	$master = array(); 
