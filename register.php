...
$layout = new TLayout("register2", "FancyCoral", "MobileCoral");
$layout->version = 2;
...
$layout = new TLayout("register_success2", "FancyCoral", "MobileCoral");
$layout->version = 2;
$layout->blocks["top"] = array();
$layout->containers["register"] = array();
$layout->container_properties["register"] = array(  );
...
//	processing _idCentro
	$control__idCentro = $pageObject->getControl("_idCentro", $id);
	$control__idCentro->readWebValue($values, $blobfields, NULL, NULL, $filename_values);

//	processing Apellido
	$control_Apellido = $pageObject->getControl("Apellido", $id);
	$control_Apellido->readWebValue($values, $blobfields, NULL, NULL, $filename_values);

//	processing Nombre
	$control_Nombre = $pageObject->getControl("Nombre", $id);
	$control_Nombre->readWebValue($values, $blobfields, NULL, NULL, $filename_values);

//	processing celular
	$control_celular = $pageObject->getControl("celular", $id);
	$control_celular->readWebValue($values, $blobfields, NULL, NULL, $filename_values);
...
//	check if entered username already exists
	...
	{
		$strSQL = "select count(*) from ". $pageObject->connection->addTableWrappers( "lcs_personal" ) 
			. " where " . 
	...
	if($retval)
	{
		$retval = DoInsertRecord("lcs_personal", $values, $blobfields, $pageObject);
	}
