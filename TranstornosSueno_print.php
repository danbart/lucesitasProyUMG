...
$page_layouts["TranstornosSueno_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=TranstornosSueno.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=TranstornosSueno.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
$xt->assign("TipoTSue_o_fieldheader", true);
...
if($mastertable == "lcs_estudiante")
{
	...
	$dparams = array("detailtable" => "TranstornosSueno", "keys" => $masterkeys);
	$master = array(); 
