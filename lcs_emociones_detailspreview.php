...
require_once("include/lcs_emociones_variables.php");
...
$page_layouts["lcs_emociones_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["idEmociones"]));
	
	
	//	idEmociones - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idEmociones", $data, $keylink);
			$row["idEmociones_value"] = $value;
			$format = $pSet->getViewFormat("idEmociones");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idEmociones")))
				$class = ' rnr-field-number';
			$row["idEmociones_class"] = $class;
	//	TipoEmocion - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TipoEmocion", $data, $keylink);
			$row["TipoEmocion_value"] = $value;
			$format = $pSet->getViewFormat("TipoEmocion");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TipoEmocion")))
				$class = ' rnr-field-number';
			$row["TipoEmocion_class"] = $class;
...
$xt->load_template(GetTemplateName("lcs_emociones", "detailspreview"));
