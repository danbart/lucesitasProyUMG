...
class CrossTableReport
{
	...
	public function isProjectDB()
	{
		...
		if("lcs_terapista" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_tiposocial" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_transtornoalimenticio" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_transtornoesfinteres" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_trastornosuenio" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_lenguaje" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_historialclinico" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_histoiralfamiliar" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_enfermedades" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_embarazo" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_espemed" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_descsocial" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_encargado" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_emociones" == $this->xml_array['tables'][0])
			$isDB = true;
		if("lcs_medicamento" == $this->xml_array['tables'][0])
			$isDB = true;
