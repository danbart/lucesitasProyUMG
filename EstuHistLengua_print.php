...
require_once("include/EstuHistLengua_variables.php");
...
$page_layouts["EstuHistLengua_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=EstuHistLengua.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=EstuHistLengua.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
if($mastertable == "lcs_centro")
{
	...
	$dparams = array("detailtable" => "EstuHistLengua", "keys" => $masterkeys);
	$master = array(); 
