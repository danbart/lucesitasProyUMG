...
require_once("include/lcs_donaciones_variables.php");
...
$page_layouts["lcs_donaciones_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["idDonaciones"]));
	
	
	//	idDonaciones - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idDonaciones", $data, $keylink);
			$row["idDonaciones_value"] = $value;
			$format = $pSet->getViewFormat("idDonaciones");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idDonaciones")))
				$class = ' rnr-field-number';
			$row["idDonaciones_class"] = $class;
	//	monto - Number
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("monto", $data, $keylink);
			$row["monto_value"] = $value;
			$format = $pSet->getViewFormat("monto");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("monto")))
				$class = ' rnr-field-number';
			$row["monto_class"] = $class;
	//	Donante - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("Donante", $data, $keylink);
			$row["Donante_value"] = $value;
			$format = $pSet->getViewFormat("Donante");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("Donante")))
				$class = ' rnr-field-number';
			$row["Donante_class"] = $class;
	//	fecha - Short Date
			$viewContainer->recId = $recordsCounter;
...
$xt->load_template(GetTemplateName("lcs_donaciones", "detailspreview"));
