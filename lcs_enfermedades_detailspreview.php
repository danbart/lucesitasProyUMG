...
require_once("include/lcs_enfermedades_variables.php");
...
$page_layouts["lcs_enfermedades_detailspreview"] = $layout;
...
if($mastertable == "Reporte Estudiantes")
{
	$where = "";
		...
		$where.=" and ";
	$where .= $pageObject->getFieldSQLDecrypt("TipoEnfermedad") . "=" . make_db_value("TipoEnfermedad",$_SESSION[$strTableName."_masterkey2"]);
}
...
if($rowcount) 
{
	...
	//	idEnfermedades - 
		    ...
		    $value = $viewContainer->showDBValue("idEnfermedades", $data, $keylink);
			$row["idEnfermedades_value"] = $value;
			$format = $pSet->getViewFormat("idEnfermedades");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idEnfermedades")))
				$class = ' rnr-field-number';
			$row["idEnfermedades_class"] = $class;
	//	TipoEnfermedad - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TipoEnfermedad", $data, $keylink);
			$row["TipoEnfermedad_value"] = $value;
			$format = $pSet->getViewFormat("TipoEnfermedad");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TipoEnfermedad")))
				$class = ' rnr-field-number';
			$row["TipoEnfermedad_class"] = $class;
...
$xt->load_template(GetTemplateName("lcs_enfermedades", "detailspreview"));
