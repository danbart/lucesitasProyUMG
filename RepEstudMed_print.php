...
$page_layouts["RepEstudMed_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=RepEstudMed.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=RepEstudMed.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
if($mastertable == "lcs_centro")
{
	...
	$dparams = array("detailtable" => "RepEstudMed", "keys" => $masterkeys);
	$master = array(); 
