...
require_once("include/TranstornosEsfinteres_variables.php");
...
$page_layouts["TranstornosEsfinteres_rprint"] = $layout;
...
if (@$_REQUEST["format"] == "excel")
{
	...
	header("Content-Disposition: attachment;Filename=TranstornosEsfinteres.xls");
...
else if (@$_REQUEST["format"] == "word")
{
	...
	header("Content-Disposition: attachment;Filename=TranstornosEsfinteres.doc");
	echo "<meta http-equiv=\"Content-Type\" content=\"text/html; charset=".$cCharset."\">";
}
...
$xt->assign("TipoEsfinteres_fieldheader", true);
...
if($mastertable == "lcs_estudiante")
{
	require_once("include/lcs_estudiante_masterprint.php");
	$masterkeys[] = @$_SESSION[$strTableName."_masterkey1"];
	$dparams = array("detailtable" => "TranstornosEsfinteres", "keys" => $masterkeys);
	$master = array(); 
	AssignFunction($master, "DisplayMasterTableInfoForPrint_lcs_estudiante", $dparams);
...
$page_layouts["lcs_estudiante_masterprint"] = $layout;
