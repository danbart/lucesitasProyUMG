...
require_once("include/lcs_transtornoesfinteres_variables.php");
...
$page_layouts["lcs_transtornoesfinteres_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["idTranstornoEsfinteres"]));
	
	
	//	idTranstornoEsfinteres - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idTranstornoEsfinteres", $data, $keylink);
			$row["idTranstornoEsfinteres_value"] = $value;
			$format = $pSet->getViewFormat("idTranstornoEsfinteres");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idTranstornoEsfinteres")))
				$class = ' rnr-field-number';
			$row["idTranstornoEsfinteres_class"] = $class;
	//	TipoEsfinteres - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TipoEsfinteres", $data, $keylink);
			$row["TipoEsfinteres_value"] = $value;
			$format = $pSet->getViewFormat("TipoEsfinteres");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TipoEsfinteres")))
				$class = ' rnr-field-number';
			$row["TipoEsfinteres_class"] = $class;
...
$xt->load_template(GetTemplateName("lcs_transtornoesfinteres", "detailspreview"));
