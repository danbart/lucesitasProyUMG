...
require_once("include/lcs_transtornoalimenticio_variables.php");
...
$page_layouts["lcs_transtornoalimenticio_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["idTranstornoAlimenticio"]));
	
	
	//	idTranstornoAlimenticio - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idTranstornoAlimenticio", $data, $keylink);
			$row["idTranstornoAlimenticio_value"] = $value;
			$format = $pSet->getViewFormat("idTranstornoAlimenticio");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idTranstornoAlimenticio")))
				$class = ' rnr-field-number';
			$row["idTranstornoAlimenticio_class"] = $class;
	//	TipoTAlimenticio - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TipoTAlimenticio", $data, $keylink);
			$row["TipoTAlimenticio_value"] = $value;
			$format = $pSet->getViewFormat("TipoTAlimenticio");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TipoTAlimenticio")))
				$class = ' rnr-field-number';
			$row["TipoTAlimenticio_class"] = $class;
...
$xt->load_template(GetTemplateName("lcs_transtornoalimenticio", "detailspreview"));
