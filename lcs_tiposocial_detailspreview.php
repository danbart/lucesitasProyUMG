...
require_once("include/lcs_tiposocial_variables.php");
...
$page_layouts["lcs_tiposocial_detailspreview"] = $layout;
...
if($rowcount) 
{
	...
	while($data && $recordsCounter<$display_count) {
		...
		$row = array();
		$keylink = "";
		$keylink.="&key1=".runner_htmlspecialchars(rawurlencode(@$data["idTipoSocial"]));
	
	
	//	idTipoSocial - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("idTipoSocial", $data, $keylink);
			$row["idTipoSocial_value"] = $value;
			$format = $pSet->getViewFormat("idTipoSocial");
			...
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("idTipoSocial")))
				$class = ' rnr-field-number';
			$row["idTipoSocial_class"] = $class;
	//	TSocial - 
			$viewContainer->recId = $recordsCounter;
		    $value = $viewContainer->showDBValue("TSocial", $data, $keylink);
			$row["TSocial_value"] = $value;
			$format = $pSet->getViewFormat("TSocial");
			$class = "rnr-field-text";
			if($format==FORMAT_FILE) 
				$class = ' rnr-field-file'; 
			if($format==FORMAT_AUDIO)
			...
			if($format==FORMAT_CHECKBOX)
				$class = ' rnr-field-checkbox';
			if($format==FORMAT_NUMBER || IsNumberType($pSet->getFieldType("TSocial")))
				$class = ' rnr-field-number';
			$row["TSocial_class"] = $class;
...
$xt->load_template(GetTemplateName("lcs_tiposocial", "detailspreview"));
